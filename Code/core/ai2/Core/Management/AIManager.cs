...
public class AIManager : GameObjectSystem<AIManager>
{
	...
	public static void NPCGo()
	{
		...
		foreach ( var npc in Current.selectedNPCs )
		{
			...
			if ( pointOnNav.HasValue )
			{
				...
				npc._scriptContext = ScriptingContext.SCRIPT_FORCED_MOVE;
				npc.inCine = true;
	...
	protected virtual void TickAISystem()
	{
		...
		foreach ( var agent in currentNPCsInScene )
		{
			...
			if ( Time.Now < agent.aiBrain._lastThinkTime + agent.aiBrain._currentThinkRate ) continue; // respect think rate
		...
		if ( Time.Now >= _nextPhysicsResolve )
		{
			_nextPhysicsResolve = Time.Now + physicsCheckRate;
