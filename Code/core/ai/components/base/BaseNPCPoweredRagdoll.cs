...
/// </summary>
public sealed class PoweredRagdoll : Component, Component.ExecuteInEditor
{
	...
	[Property, Group( "Physics" )]
	public float DriveFromAnimationInfluence { get; set; }
	...
	private void CreatePhysics()
	{
		...
		foreach ( var part in physics.Parts )
		{
			...
			body.MotionEnabled = RagdollEnabled;
	...
	private void DriveBodyFromAnimation( Body body )
	{
		...
		if ( Renderer.TryGetBoneTransformAnimation( body.Bone, out var boneWorld ) )
		{
			...
			var currentRotation = body.Component.WorldRotation;

			// Interpolate between current rotation and target rotation based on strength
			var targetRotation = boneWorld.Rotation;
			var blendedRotation = Rotation.Slerp( currentRotation, targetRotation, DriveFromAnimationInfluence );

			// Smoothly rotate towards the blended rotation based on strength
			body.Component.SmoothRotate( blendedRotation, 0.01f, Time.Delta );
		}
	}
