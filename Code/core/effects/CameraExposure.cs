...
public class CameraExposure : BaseEntity, Component.ExecuteInEditor
{
	...
	[DebugExpose, Property, ReadOnly, Feature( "Debug" )] public float ResultExposure { get; private set; } = 1f;

	[DebugExpose, Property, ReadOnly, MakeDirty] public float RealEV { get; set; } = 0;
	...
	[DebugExpose, Property, MakeDirty] public ExposureMode Mode { get; set; } = ExposureMode.Manual;
	[Space]

	// Manual

	// Default settings are set for the Sunny 16
	[DebugExpose, MakeDirty, Header( "Settings" ), Range( 100, 1600 ), Step( 100 ), Property, Title( "ISO" ), ShowIf( nameof( Mode ), ExposureMode.Manual )]
	public float ISO { get; set; } = 100f;
	[DebugExpose, MakeDirty, Property, Title( "Shutter Speed (1 / float)" ), Range( 30, 1000 ), Step( 10 ), ShowIf( nameof( Mode ), ExposureMode.Manual )]
	public float Shutter { get; set; } = 100f;
	[DebugExpose, MakeDirty, Property, Title( "Aperture" ), Range( 1.4f, 22f ), Step( 0.1f ), ShowIf( nameof( Mode ), ExposureMode.Manual )]
	public float Aperture { get; set; } = 16f;

	// Manual with Zoning
	[DebugExpose, MakeDirty, Header( "Settings" ), Property, Title( "Target EV" ), Range( -4, 16 ), ShowIf( nameof( Mode ), ExposureMode.ManualZoning )]
	public float TargetEV { get; set; } = 0;

	[MakeDirty, Property, Title( "Sun ND" ), Header( "Hacks" ), Description( "Cheap hack to expose the sky correctly in cases where it sucks" ), Range( 0, 1 ), Step( 0.1f )] public float SunND { get; set; } = 1f;
	...
	[DebugExpose, Property, Range( 0, 1 ), Feature( "Debug" ), Change( nameof( BlendChange ) ), ReadOnly] private float Blend { get; set; }
